...
set N :=T cross P;
set TT:=T cross T;
var A[M] binary;#Ai,k表示当任务i被映射到处理器k时取1，就是对应此处的M，T-C，也就是任务到处理器
var PI[N] binary;#任务i有优先级p时取1，所以对应此处的N，T-P
var V[TT] binary;#是不是一个处理器，是一个处理器是取1
var X[TT] binary;#前面的是不是比后面的有更高的优先级，是的话取1
var RT[T] integer >= 0;
var OFF[T] integer >= 0;
