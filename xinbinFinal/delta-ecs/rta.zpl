set T:={ read "tasks2.txt" as "<1s>" comment "#" };
set C:={ read "processors2.txt" as "<1s>" comment "#" };
set P:={ 1 to card(T)};
param period[T]:=read "tasks2.txt" as "<1s> 2n" comment "#";
param jitter[T]:=read "tasks2.txt" as "<1s> 5n" comment "#";
param wcet[T]:=read "tasks2.txt" as "<1s> 3n" comment "#";
param bcet[T]:=read "tasks2.txt" as "<1s> 4n" comment "#";
set M :=T cross C;
set N :=T cross P;
set TT:=T cross T;
var A[M] binary;
...
subto samecore_high_2:
	forall <i> in T:
		forall <x> in T - {i}:
				K[i,x] >= 1 - (2 - V[i,x] - X[i,x]);
subto samecore_high:
	forall <i> in T:
		forall <x> in T - {i}:
			K[i,x] + K[x,i] <= V[i,x];
